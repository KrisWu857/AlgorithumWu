...
public class Code01_CreateGraph {
    ...
    // 邻接矩阵方式建图
    public static int[][] graph1 = new int[MAXN][MAXM];
    ...
    public static void  build(int n ) {
        ...
        //邻接表的清空
        graph2.clear();
        // 下标需要支持1~n , 所以要加入n + 1 个列表, 0下标准备但不使用
        for (int i = 0 ; i <= n ; i++) {
    ...
    public static void addEdge(int u , int v ,int w ) {
        ...
        next[cnt] = head[u];
        to[cnt] = v ;  // 现在这一条边去v
        weight[cnt] = w ;  // 现在这一条边的权重 是w
        head[u] = cnt++;  // 出发点头部的边就是cnt的编号的边
    }
...
//            graph2.get(edge[1]).add(edge[0]);
            graph2.get(edge[0]).add(new int[] {edge[0], edge[1]} );
            graph2.get(edge[0]).add(new int[] {edge[1], edge[0]} );
        }
