...
public class Code06_FlightPath1 {
    ...
    public static int dijkstra(){
        ...
        while (!heap.isEmpty()) {
            ...
            for (int ei = head[uId], v, w; ei > 0 ; ei = next[ei]) {
                v = to[ei];
                w = weight[ei];
                if ( use  <  k && distance[v][ use + 1 ] > distance[uId][use]) {
                    // 使用免费
                    distance[uId][use + 1 ] = distance[uId][use] ;
                    heap.add(new int[] { v , use + 1 , distance[v][use + 1 ]});
                }
                if (distance[v][use] > distance[uId][use] + w ) {
                    // 不使用免费
                    distance[v][use] = distance[uId][use] + w ;
